...
class TransferLinkLocalServerService : Service() {
    private val transferLinkLocalServerRepository: TransferLinkLocalServerRepository by inject()
    ...
    override fun onStartCommand(
        ...
        when (intent?.action) {
            ACTION_START -> {
                ...
                serviceScope.launch {
                    transferLinkLocalServerRepository.startServer()
                    observeServerState()
                }
            }
            ...
            ACTION_STOP -> {
                serviceScope.launch {
                    transferLinkLocalServerRepository.stopServer()
    ...
    private fun observeServerState() {
        serviceScope.launch {
            transferLinkLocalServerRepository.isRunning.collect { isRunning ->
                if (isRunning) {
                    val serverUrl = transferLinkLocalServerRepository.serverUrl.first()
    ...
    private fun createNotification(serverUrl: String?): Notification {
        ...
        val stopIntent =
            Intent(this, TransferLinkLocalServerService::class.java).apply {
                action = ACTION_STOP
            }
    ...
    companion object {
        ...
        const val ACTION_START = "com.yogeshpaliyal.deepr.ACTION_TRANSFER_LINK_START_SERVER"
        const val ACTION_STOP = "com.yogeshpaliyal.deepr.ACTION_TRANSFER_LINK_STOP_SERVER"

        fun startService(context: Context) {
            val intent =
                Intent(context, TransferLinkLocalServerService::class.java).apply {
                    action = ACTION_START
                }
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
